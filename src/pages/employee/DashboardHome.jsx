...
=======
import { WORK_MODES, WORK_MODE_LABELS, SHIFT_CONFIG } from '../../lib/constants';
>>>>>>> Stashed changes
import {
  CalendarCheck,
  CalendarDays,
...
=======
      // 3. Fetch Employee Leave Requests
>>>>>>> Stashed changes
...
=======
  // Handle Admin Quick Approve / Reject Leave
  const handleAdminReviewLeave = async (leaveId, status) => {
    try {
      await leaveService.updateLeaveStatus(leaveId, {
        status,
        comments: status === 'approved' ? 'Approved by Admin' : 'Declined per team schedule'
      });
...
=======
    <div className="space-y-6 animate-fade-in text-zinc-900 dark:text-zinc-100">
      {/* ─────────────────────────────────────────────────────────────
          1. EMPLOYEE NOTIFICATIONS SECTION (AT THE VERY TOP)
      ───────────────────────────────────────────────────────────── */}
      {notifications.length > 0 && (
        <div className="space-y-2">
          <div className="flex items-center justify-between px-1">
            <h2 className="text-xs font-bold uppercase tracking-wider text-zinc-600 dark:text-zinc-400 flex items-center gap-1.5">
              ...
              Employee Notifications ({notifications.length})
            </h2>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {notifications.map((notif) => (
              ...
              >
                ...
                <button
                  onClick={() => handleDismissNotification(notif.id)}
                  className="text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 p-1 rounded-lg transition"
                  title="Dismiss notification"
                >
            ...
            <div className="flex items-center justify-between pb-3 border-b border-zinc-100 dark:border-zinc-800">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-zinc-500">Track application status in real-time</p>
                </div>
              </div>
                ...
                leaves.slice(0, 3).map((leave) => (
                  <div key={leave.id} className="py-3 flex items-center justify-between gap-3">
                    ...
                    <div className="text-right shrink-0">
                      <Badge variant={leave.status}>{leave.status}</Badge>
                      {leave.comments && (
                        <p className="text-[10px] text-teal-700 dark:text-teal-300 mt-0.5 truncate max-w-[120px]" title={leave.comments}>
                          {leave.comments}
                        </p>
                      )}
                    </div>
...
=======
              View All Leave History <ArrowRight className="w-3 h-3" />
>>>>>>> Stashed changes
            </Link>
          </div>
        </Card>
...
=======
        ...
        subtitle="Apply for Paid Time Off (PTO), Sick Leave, or Casual Leave"
>>>>>>> Stashed changes
      >
...
=======
              placeholder="e.g. Attending a family function out of town..."
>>>>>>> Stashed changes
              className="w-full p-3 rounded-xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-950 text-sm focus:border-teal-600"
            />
          </div>
...
<<<<<<< Updated upstream
            <Button type="submit" variant="primary" loading={submittingLeave} className="bg-teal-600 hover:bg-teal-700 font-bold">
              Submit Leave Request
=======
            <Button type="submit" variant="primary" loading={submittingLeave} className="bg-teal-600 hover:bg-teal-700">
              Submit Application
>>>>>>> Stashed changes
            </Button>
          </div>
